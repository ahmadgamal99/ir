...
class StopWordRemovalController extends Controller
{
    public static  $stopWordList = [ "a", "about", "above", "after", "again", "against", "all", "am", "an", "and", "any", "are", "aren't", "as",
        ...
        "out", "over", "own", "same", "shan't", "she", "she'd", "she'll", "she's", "should", "shouldn't", "so", "some", "such", "than", "that", "that's",
        "the", "their", "theirs", "them", "themselves", "then", "there", "there's", "these", "they", "they'd", "they'll", "they're", "they've", "this",
    ...
    {
        $tokensWithDocID = $this->stopWordRemoval();
        return view('stop-word-removal-result',compact('tokensWithDocID'));
    ...
    public function stopWordRemoval()
    {
        ...
        $tokensWithDocID = $tokenizer->tokensWithDocID();

        foreach ($tokensWithDocID as $key => $item) {
            if(in_array($item['token'], self::$stopWordList)){
                unset($tokensWithDocID[$key]);
            }
        }
